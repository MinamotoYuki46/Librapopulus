...
<body class="bg-gray-200 min-h-screen">
    ...
    <main class="px-6 pb-6 py-6" id="mainContent">
        <div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-md mt-10">
            <h1 class="text-3xl font-bold text-gray-800 mb-6">Edit Grup</h1>
            ...
            <form action="<?= base_url('group/proceedEditGroup/' . $group['slug']) ?>" method="POST" enctype="multipart/form-data" class="space-y-5" id="groupForm">
                <?= csrf_field() ?>

                <div class="mb-6">
                    <label for="group_name" class="block text-gray-700 text-sm font-bold mb-2">Nama Grup</label>
                    <input type="text" name="group_name" id="group_name" value="<?= old('group_name', $group['name']) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                </div>

                <div class="mb-6">
                    <label for="description" class="block text-gray-700 text-sm font-bold mb-2">Deskripsi (Opsional)</label>
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><?= old('description', $group['description']) ?></textarea>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="flex items-center space-x-6">
                        <div class="shrink-0">
                            <img id="icon_preview" class="h-20 w-20 object-cover rounded-full" 
                                 src="<?= $group['icon'] ? base_url('uploads/groups/' . $group['icon']) : 'https://via.placeholder.com/150' ?>" 
                                 alt="Pratinjau Ikon Grup" />
                        </div>
                        <label class="block">
