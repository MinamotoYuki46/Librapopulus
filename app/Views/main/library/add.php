    ...
    <main class="max-w-xl mx-auto bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        <form action="<?= base_url('library/proceedAddBook') ?>" method="POST" enctype="multipart/form-data" class="space-y-5" id="bookForm">
            ...
            <div>
                ...
                <select id="genre_id" name="genre_id" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
                    <option value="" disabled selected>Pilih genre</option>
                    <?php foreach ($genres as $genre): ?>
                        <option value="<?= esc($genre['id']) ?>"><?= esc($genre['genre_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
